    ...
    html += `
      <div class="info-line expense-line">
        <span class="expense-main">
          <span class="budget-tag">${expense.ville}</span>
          ${expense.d}
        </span>
        <strong>${expense.a} ${expense.c}</strong>
        <button class="expense-delete-btn" data-expense-index="${index}">✕</button>
      </div>
    `;
  });
...
function addExpense() {
  ...
  const currencyInput = document.getElementById("exp-currency");

  const amount = amountInput?.value?.trim();
  const desc = descInput?.value?.trim() || "Dépense";
  const currency = currencyInput?.value || "VND";
...
function convertEurToVnd() {
  ...
  const vndInput = document.getElementById("conv-vnd");
  if (vndInput) vndInput.value = Math.round(eur * state.exchangeRate);
}
...
function convertVndToEur() {
  ...
  if (eurInput) eurInput.value = (vnd / state.exchangeRate).toFixed(2);
}
